...
//            ROS_INFO("TIMESTAMP: %f",point_rs.timestamp);
    ...
    void Initframe(pcl::PointCloud<PointXYZIRT>::Ptr cloud, Sophus::SE3d begin_pose, Sophus::SE3d end_pose,
                   double timeB, double timeE)
    {
        curr_frame.Reset();
...
//        }
    ...
    void getSegCloud(double timeSegBegin, double timeSegEnd,
                     pcl::PointCloud<PointXYZIRT>::Ptr SegCloud){
        SegCloud->clear();
...
//                    std::vector<Eigen::Vector3d > neighbor;
                ...
                double alpha = (point.timestamp - curr_frame.timeStart)/( curr_frame.timeEnd-curr_frame.timeStart);
...
//            pcl::copyPointCloud(*cloud_valid,*curr_frame.cloud_ori_downsample );
    ...
    void ProcessSeg(double  timeSegBegin, double timeSegEnd){

        getSegCloud(timeSegBegin, timeSegEnd,SegCloud);
        ...
        Initframe(SegCloud, begin_pose,end_pose, timeSegBegin, timeSegEnd);
...
//           ---------------------------------------------------init------------------------------------
            ...
            //process b-half
            ProcessSeg(timeStart, timeMid);

            //process e-half
            ROS_INFO("-------------------------half----------");
            ProcessSeg(timeMid, timeEnd);
