    ...
    void preprocess()
    {
        ...
        while (ros::ok())
        {
            ...
            if(debug_print) {
                ...
                ROS_INFO("AFTER DOWNSAMPLE: %d", curr_frame.cloud_ori_downsample->points.size());
            }


            //--------------find neighbor----------------------
            double findNeighbor_average=0;
...
//#pragma omp parallel for num_threads(8)
                for (auto point : curr_frame.cloud_ori_downsample->points) {
                    ...
                    SE3 inter_pose = curr_frame.poses.linearInplote(alpha);
...
//                                new CTFunctor2(alpha, raw_point, getPointXYZ(neighbor.points[0]), weight,neighbor.description.normal));
                        ceres::CostFunction *cost_function = new ceres::AutoDiffCostFunction<CTFunctor3, 1, 4, 4,4,3, 3, 3>(
                                new CTFunctor3(alpha, raw_point, getPointXYZ(neighbor.points[0]), weight,normal));
                ...
                findNeighbor_average = std::chrono::duration<double,std::milli >(
                        findNeighbor_end - findNeighbor_start).count();
...
//                }
                ...
                problem.AddResidualBlock(new ceres::AutoDiffCostFunction<ConstantVelocityRotTran,6,4,4,3,3>(
                                         ...
                                         nullptr,
                                         begin_quat.coeffs().data(),mid_quat.coeffs().data(),begin_trans.data(),mid_trans.data()
                );

                problem.AddResidualBlock(new ceres::AutoDiffCostFunction<MultiModeConstantVelocity,6,4,4,4,3,3,3>(
                                                 new MultiModeConstantVelocity(std::sqrt(opt_num*0.01))),
                                         nullptr,
                                         begin_quat.coeffs().data(),mid_quat.coeffs().data(),end_quat.coeffs().data(),
                                         begin_trans.data(),mid_trans.data(),end_trans.data()
                );
...
//            ROS_INFO("ruler: %f, %f,%f", end_euler.roll, end_euler.pitch, end_euler.yaw);
            ...
            if(debug_print){
                ROS_INFO("cloud find neighbor COST: %f ms",findNeighbor_average);
                ROS_INFO("SLOVE COST: %f ms",solve_average);
