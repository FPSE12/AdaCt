...
//            if(edge_map.NeighborSearch(curr_frame.edge_world->points[i],0.8,neighbor)){
//
//                //opt_edge_num++;
    ...
    void AddPlaneProblem(ceres::Problem &problem,ceres::LossFunction *loss_function ,Eigen::Quaterniond &begin_quat, Eigen::Quaterniond &end_quat, Eigen::Vector3d & begin_trans, Eigen::Vector3d & end_trans){
        for(int i=0;i<curr_frame.planeDS->size();i++){
           ...
           // Eigen::Vector3d  sensor_location = (1- alpha) * begin_trans + alpha * end_trans;
            VoxelBlock<PointXYZIRT> neighbor;
            Eigen::Vector4d pabcd;
//            if( plane_map.NeighborSearch(curr_frame.plane_world->points[i],0.8,neighbor)){
...
//                                new CTFunctor(alpha, raw_point, pabcd, weight));
    ...
    void preprocess()
    {
        ...
        while (ros::ok())
        {
            ...
            if (edgeCloudQue.size()==0 || planesCloudQue.size()==0 || fullCloudQue.size()==0 || odoInitQue.empty())
            {
                continue;
            }
...
//            Eigen::Vector3d curr2last_trans(0,0,0);
            ...
            laserOdometryTrans.setOrigin(tf::Vector3(0,0,0));
