...
#include "AdaCt/sophus_utils.hpp"
...
//    frame_info curr_frame;
    ...
    //slide_window
    int windows_capability=2;
    std::deque<frame_info> window_frames;
    std::deque<OptPose> window_poses;

    //marg
    Eigen::MatrixXd marg_H;
    Eigen::VectorXd marg_B;
    double init_pose_weight = 1e9;
    Eigen::VectorXd update_all;
...
//            }
                   ...
                   int frame_id)
    {
        ...
        curr_frame.predict_pose.begin_pose = begin_pose;
        curr_frame.predict_pose.end_pose = end_pose;
    ...
    bool Solve(frame_info & curr_frame, int & motion_evaluate, int seg_num){
        ...
        Eigen::MatrixXd abs_H_noMarg;
        Eigen::VectorXd abs_B_noMarg;
        ...
        for (; iter_count < max_iter_nums; iter_count++) {
            abs_H.setZero(12,12);
            abs_B.setZero(12);
            abs_H_noMarg.setZero(12,12);
            abs_B_noMarg.setZero(12);
            Sophus::SE3d T_be = curr_frame.pose.begin_pose.inverse() * curr_frame.pose.end_pose;
            Sophus::Vector6d delta_ = Sophus::se3_logd(T_be);
            Eigen::Matrix3d R_w_b_t = curr_frame.pose.begin_pose.rotationMatrix().transpose();
            Eigen::Matrix3d R_w_e_t = curr_frame.pose.end_pose.rotationMatrix().transpose();
            Eigen::Matrix3d R_e_b = R_w_e_t * curr_frame.pose.begin_pose.rotationMatrix();
...
#pragma omp parallel for
            for(auto &p: curr_frame.points){
                ...
                Sophus::SE3d T_w_i = curr_frame.pose.linearInplote(p.alpha);
//                Sophus::SE3d T_w_i = curr_frame.pose.begin_pose *Sophus::se3_expd(p.alpha * delta_);
                p.point_world = T_w_i * p.point;

                ptpl pl;
                bool isvalid = BuildResidulSingleOMP(voxel_map,max_voxel_size,3.0,max_layer, p,pl,dis_thres);
...
//                    continue;
//                }
                    double weight = Weight((pl.point_world - pl.plane_point).squaredNorm());
                    Eigen::Vector3d normal = pl.normal.normalized();
                    if(normal.dot(pl.point_world - pl.plane_point)<0){
                        normal = -normal;
                    }

                    Eigen::Matrix3d Information = normal * normal.transpose();

                    //Sophus::SE3d T_w_i = curr_frame.pose.linearInplote(alpha);
                    ...
                    J_T_wi.block<3, 3>(0, 3) =
                            -T_w_i.rotationMatrix() * Sophus::SO3d::hat(pl.point);//9(b)，采用的是右扰动模型

                    Eigen::Matrix3d Jr;
                    Eigen::Matrix3d Jr_inv;

                    Eigen::Vector3d omega = delta_.tail<3>();//tail is rot
                    ...
                    abs_H_noMarg += weight * J_r.transpose() * Information * J_r;//H
                    abs_B_noMarg -= weight * J_r.transpose() * normal * abs(normal.dot(pl.point_world - pl.plane_point));//
                    ++opt_num;
                }
            ...
            {
                Sophus::Vector6d res = delta_ - Sophus::se3_logd(last_pose.begin_pose.inverse()* last_pose.end_pose);//当前帧的变化 减去 之前的预测值
                Sophus::Matrix6d J_T_w_b;
                Sophus::Matrix6d J_T_w_e;
                Sophus::Matrix6d rr_b;
                Sophus::Matrix6d rr_e;

                Sophus::rightJacobianInvSE3Decoupled(delta_, J_T_w_e);
                J_T_w_b = -J_T_w_e * (T_be.inverse()).Adj();

                rr_b.setIdentity();
                rr_b.topLeftCorner<3, 3>() = R_w_b_t;
                rr_e.setIdentity();
                rr_e.topLeftCorner<3, 3>() = R_w_e_t;

                Eigen::Matrix<double, 6, 12> J_be;
                J_be.topLeftCorner<6, 6>() = J_T_w_b * rr_b;
                J_be.topRightCorner<6, 6>() = J_T_w_e * rr_e;

                double alpha_e = kinematic_ *opt_num;
                //ROS_INFO("OPT_NUM;%d",opt_num);
                abs_H_noMarg += alpha_e * J_be.transpose() * J_be;
                abs_B_noMarg -= alpha_e * J_be.transpose() * res;
            }
...
//            abs_H.block<6, 6>(0, 0) += marg_H;
//            abs_B.head<6>() -= marg_B;
//            abs_B.head<6>() -= (marg_H * update_all.head<6>());

            if(iter_count ==0){
                Eigen::VectorXd abs_H_eigen_val(12);
                Eigen::MatrixXd abs_H_eigen_vec(12, 12);
                Eigen::MatrixXd abs_H_eigen_vec_2(12, 12);
                std::vector<double> eigen_thr(12, 10);

                Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> es(abs_H);

                abs_H_eigen_vec = es.eigenvectors();
                abs_H_eigen_vec_2 = abs_H_eigen_vec;
                abs_H_eigen_val = es.eigenvalues();

                //ROS_WARN("MIN_EIGEN : %f", abs_H_eigen_val[0]);
                if(abs_H_eigen_val[0]<degenrated_thres){
                   // ROS_WARN("MIN_EIGEN : %f", abs_H_eigen_val[0]);
                    isDegenerate = true;
                }
...
//                        abs_H_eigen_vec_2.col(i).setZero();
//                        isDegenerate = true;
//                    }
//                }
//                update_P = abs_H_eigen_vec.inverse() * abs_H_eigen_vec_2;

                if(isDegenerate){
//                    ds_size_ = ds_size_ /(2.0);
//                    if(ds_size_ < 0.05){
//                        //kinematic_ *= 1.5;
//                        goto CONTINUE_SOLVE;
//                        ROS_WARN("continue");
...
//                    return false;
        ...
        if (Debug_print) {
                    ...
                    ROS_INFO("compute COST: %f ms",
                             solve_cost);
                    ROS_INFO("iter all COST: %f ms",
                             std::chrono::duration<double, std::milli>(iter_end - iter_start).count());
//                    ROS_INFO("OPT_NUM:%d",opt_num);

        }

        abs_H_noMarg.topLeftCorner<6, 6>() += marg_H;

        abs_B_noMarg.head<6>() -= marg_B;
        abs_B_noMarg.head<6>() -= (marg_H * update_all.head<6>());

        // 舒尔补的公共矩阵
        Eigen::MatrixXd H_mm_inv =
                abs_H_noMarg.topLeftCorner<6, 6>().fullPivLu().solve(Eigen::MatrixXd::Identity(6, 6));
        abs_H_noMarg.bottomLeftCorner<6, 6>() *= H_mm_inv;

        // 舒尔补
        marg_H = abs_H_noMarg.bottomRightCorner<6, 6>();
        marg_B = abs_B_noMarg.tail<6>();
        marg_H -= abs_H_noMarg.bottomLeftCorner<6, 6>() * abs_H_noMarg.topRightCorner<6, 6>();
        marg_B -= abs_H_noMarg.bottomLeftCorner<6, 6>() * abs_B_noMarg.head<6>();
    ...
    void ProcessSeg( std::vector<PointType> & points_vec, double timeStart, double timeEnd,
        ...
        ds_size_ = downsampleSize;
        kinematic_ = kinematic_factor;
        Initframe(curr_frame, SegPoints, begin_pose,end_pose, timeSegBegin, timeSegEnd, headertime, frame_id);

DOWNSAMPLE_CURR_FRAME:
        curr_frame.grid_sample_mid_in_vec(ds_size_);
        auto init_end = std::chrono::steady_clock::now();

        auto solve_begin = std::chrono::steady_clock::now();
        bool success= Solve(curr_frame, motion_evaluate,seg_num);
        if(!success){

            goto DOWNSAMPLE_CURR_FRAME;
        }
        auto solve_end = std::chrono::steady_clock::now();
...
//            preprocess(cloud_ori, points_vec,headertime,timeStart, timeEnd);
            ...
            int seg_num = 2;
            double seg_length = (timeEnd - segTimeStart)/((double)seg_num);
            double seg_start = segTimeStart;
            double seg_end  = segTimeStart;
            for(int seg_index=0;seg_index<seg_num;seg_index++){
               seg_start = seg_end;
               seg_end = min(seg_start + seg_length, timeEnd);
               ProcessSeg(points_vec,segTimeStart, timeEnd, seg_start, seg_end, headertime,
