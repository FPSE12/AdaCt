...
class FeatureAssociation: public configParam{
...
private:
    ...
    std::deque<sensor_msgs::PointCloud2> segment_queue;
    std::deque<sensor_msgs::PointCloud2> outliers_queue;
    std::deque<sensor_msgs::PointCloud2> fullcloud_queue;
    std::deque<AdaCt::cloud_info> cloudinfo_queue;
    ...
    sensor_msgs::PointCloud2 fullCloudROS;

    pcl::PointCloud<PointXYZIRT>::Ptr cornerPointsSharp;
    pcl::PointCloud<PointXYZIRT>::Ptr cornerPointsLessSharp;
    pcl::PointCloud<PointXYZIRT>::Ptr surfPointsFlat;
    pcl::PointCloud<PointXYZIRT>::Ptr surfPointsLessFlat;

    pcl::PointCloud<PointXYZIRT>::Ptr surfPointsLessFlatScan;
    pcl::PointCloud<PointXYZIRT>::Ptr surfPointsLessFlatScanDS;
    ...
    ros::Publisher pubFullCloudLast;
    ...
    pcl::PointCloud<PointXYZIRT>::Ptr laserCloudCornerLast;
    pcl::PointCloud<PointXYZIRT>::Ptr laserCloudSurfLast;
    pcl::PointCloud<PointXYZIRT>::Ptr laserCloudOri;
    pcl::PointCloud<PointXYZIRT>::Ptr coeffSel;

    pcl::KdTreeFLANN<PointXYZIRT>::Ptr kdtreeCornerLast;
    pcl::KdTreeFLANN<PointXYZIRT>::Ptr kdtreeSurfLast;

    std::vector<int> pointSearchInd;
    std::vector<float> pointSearchSqDis;

    PointXYZIRT pointOri, pointSel, tripod1, tripod2, tripod3, pointProj, coeff;
...
public:
    ...
    FeatureAssociation()
    {
        ...
        subLaserCloudInfo = nh.subscribe<AdaCt::cloud_info>("/segmented_cloud_info", 1, &FeatureAssociation::laserCloudInfoHandler, this);
        subOutlierCloud = nh.subscribe<sensor_msgs::PointCloud2>("/outlier_cloud", 1, &FeatureAssociation::outlierCloudHandler, this);
        subFullCloud = nh.subscribe<sensor_msgs::PointCloud2>("/full_cloud_projected",1,&FeatureAssociation::fullCloudHandler,this);
        ...
        pubLaserOdometry = nh.advertise<nav_msgs::Odometry> ("/laser_odom_to_init", 1);
        pubFullCloudLast = nh.advertise<sensor_msgs::PointCloud2>("/fullCloudLast",2);
    ...
    void initializationValue()
    {
        ...
        cornerPointsSharp.reset(new pcl::PointCloud<PointXYZIRT>());
        cornerPointsLessSharp.reset(new pcl::PointCloud<PointXYZIRT>());
        surfPointsFlat.reset(new pcl::PointCloud<PointXYZIRT>());
        surfPointsLessFlat.reset(new pcl::PointCloud<PointXYZIRT>());

        surfPointsLessFlatScan.reset(new pcl::PointCloud<PointXYZIRT>());
        surfPointsLessFlatScanDS.reset(new pcl::PointCloud<PointXYZIRT>());
        ...
        laserCloudCornerLast.reset(new pcl::PointCloud<PointXYZIRT>());
        laserCloudSurfLast.reset(new pcl::PointCloud<PointXYZIRT>());
        laserCloudOri.reset(new pcl::PointCloud<PointXYZIRT>());
        coeffSel.reset(new pcl::PointCloud<PointXYZIRT>());

        kdtreeCornerLast.reset(new pcl::KdTreeFLANN<PointXYZIRT>());
        kdtreeSurfLast.reset(new pcl::KdTreeFLANN<PointXYZIRT>());
    ...
    void TransformToStartIMU(PointXYZIRT *p)
    {
...
//    }
    ...
    void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg){
        ...
        segment_queue.push_back(*laserCloudMsg);
...
//        newSegmentedCloud = true;
    ...
    void outlierCloudHandler(const sensor_msgs::PointCloud2ConstPtr& msgIn){
        //ROS_INFO("2");
        std::lock_guard<std::mutex> lock(mtx);
        outliers_queue.push_back(*msgIn);
...
//        newOutlierCloud = true;
    ...
    void laserCloudInfoHandler(const AdaCt::cloud_infoConstPtr& msgIn)
    {
        //ROS_INFO("3");
        std::lock_guard<std::mutex> lock(mtx);
        cloudinfo_queue.push_back(*msgIn);
...
//        newSegmentedCloudInfo = true;
    ...
    void fullCloudHandler(const sensor_msgs::PointCloud2::ConstPtr& msgIn){
        std::lock_guard<std::mutex> lock(mtx);
        fullcloud_queue.push_back(*msgIn);

//        fullCloud->clear();
//        pcl::fromROSMsg(*msgIn, *fullCloud);
    }

    bool copyCloud(){

        if(outliers_queue.empty() || cloudinfo_queue.empty() ||segment_queue.empty() || fullcloud_queue.empty()){
            return  false;
        }

        //ROS_INFO("1");
        cloudHeader = segment_queue.front().header;

        timeScanCur = cloudHeader.stamp.toSec();
        timeNewSegmentedCloud = timeScanCur;

        segmentedCloud->clear();
        pcl::fromROSMsg(segment_queue.front(), *segmentedCloud);

        newSegmentedCloud = true;

        timeNewOutlierCloud = outliers_queue.front().header.stamp.toSec();

        outlierCloud->clear();
        pcl::fromROSMsg(outliers_queue.front(), *outlierCloud);

        newOutlierCloud = true;


        timeNewSegmentedCloudInfo = cloudinfo_queue.front().header.stamp.toSec();
        segInfo = cloudinfo_queue.front();
        newSegmentedCloudInfo = true;

        timeNewFullCloud = fullcloud_queue.front().header.stamp.toSec();
        fullCloud->clear();
        pcl::fromROSMsg(fullcloud_queue.front(), *fullCloud);
        return true;
    }
    ...
    void adjustDistortion()
    {
        ...
        for (int i = 0; i < cloudSize; i++) {
            ...
            point.intensity = int(segmentedCloud->points[i].intensity) + 0.1 * relTime;
            point.timestamp = segmentedCloud->points[i].timestamp;

            if (imuPointerLast >= 0) {
                float pointTime = relTime * 0.1;
                ...
                if (timeScanCur + pointTime > imuTime[imuPointerFront]) {//IMU数据还没有到达这个点的时间戳，就只能用最后一个IMU
                ...
                if (i == 0) {//第一个激光点，设置为IMU积分基准
                    ...
                    if (timeScanCur + pointTime > imuTime[imuPointerFront]) {//imu还没到
    ...
    void markOccludedPoints()
    {
        ...
        for (int i = 5; i < cloudSize - 6; ++i){
            ...
            //靠的近
            if (columnDiff < 10){
                //距离又差的远（把远的点周围设为1，提取特征时不会被提取，因为本体运动时，雷达会运动比较剧烈）
            ...
            for (int j = 0; j < 6; j++) {
                ...
                for (int k = ep; k >= sp; k--) {
                    ...
                    if (cloudNeighborPicked[ind] == 0 &&
                        ...
                        segInfo.segmentedCloudGroundFlag[ind] == false) {//没有被挑选过+大于阈值+不能是地面
                ...
                for (int k = sp; k <= ep; k++) {
                    ...
                    if (cloudNeighborPicked[ind] == 0 &&
                        ...
                        segInfo.segmentedCloudGroundFlag[ind] == true) {//没被挑选过，小于阈值，可以是地面
...
//            *surfPointsLessFlat += *surfPointsLessFlatScanDS;
    ...
    void publishCloud()
    {
        ...
        if (pubCornerPointsSharp.getNumSubscribers() != 0){
            ...
            laserCloudOutMsg.header.frame_id = "odometry";
        ...
        if(pubFullCloudLast.getNumSubscribers()!=0){
            pcl::toROSMsg(*fullCloud,laserCloudOutMsg);
            laserCloudOutMsg.header.stamp = cloudHeader.stamp;
            laserCloudOutMsg.header.frame_id = "odometry";
            pubFullCloudLast.publish(laserCloudOutMsg);
        }
    }
...
//------------------------------------------------------------------------------------------------
    void TransformToStart(PointXYZIRT const * const pi, PointXYZIRT * const po)
    {
    ...
    void TransformToEnd(PointXYZIRT const * const pi, PointXYZIRT * const po)
    {
        float s = 10 * (pi->intensity - int(pi->intensity));
    ...
    bool calculateTransformationSurf(int iterCount){
        ...
        if (iterCount == 0) {
            ...
            //根据ATA的特征值和特征向量判断是否退化，特征值递减
            cv::eigen(matAtA, matE, matV);
            matV.copyTo(matV2);

            isDegenerate = false;
            float eignThre[3] = {10, 10, 10};
            for (int i = 2; i >= 0; i--) {//小于10发生退化，置0
    ...
    void checkSystemInitialization(){
        ...
        pcl::PointCloud<PointXYZIRT>::Ptr laserCloudTemp = cornerPointsLessSharp;
    ...
    void updateInitialGuess(){
        ...
        if (imuVeloFromStartX != 0 || imuVeloFromStartY != 0 || imuVeloFromStartZ != 0){
            transformCur[3] -= imuVeloFromStartX * 0.1;
            transformCur[4] -= imuVeloFromStartY * 0.1;
            transformCur[5] -= imuVeloFromStartZ * 0.1;
        }
    }
...
//        tfBroadcaster.sendTransform(laserOdometryTrans);
    ...
    void adjustOutlierCloud(){
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            point.timestamp = outlierCloud->points[i].timestamp;
...
//            TransformToEnd(&surfPointsLessFlat->points[i], &surfPointsLessFlat->points[i]);// change to world axis
//        }
...
//        }
    ...
    void runFeatureAssociation()
    {
       ...
       if(!copyCloud()) return ;
        ...
        cloudinfo_queue.pop_front();
        segment_queue.pop_front();
        fullcloud_queue.pop_front();
        outliers_queue.pop_front();
...
//        ROS_INFO("%f",cornerPointsLessSharp->points[0].timestamp);
//        ROS_INFO("%f", surfPointsLessFlat->points[0].timestamp);
        publishCloud(); // cloud for visualization

        //20ms
        auto end_1 = std::chrono::steady_clock::now();
        ROS_INFO("extraction: %f ms",
        ...
        */
        ...
        ROS_INFO("association: %f ms",
                 std::chrono::duration<double, std::milli>(end_2 - start_2).count());
    }
};
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO(" Feature Association Started.");
